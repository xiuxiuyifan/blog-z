...
起因：Diff算法无法避免新旧虚拟DOM中无用的比较操作，通过patchFlags来标记动态内容，可以实现快速diff算法
...
return function render(_ctx, _cache, $props, $setup, $data, $options) {
  return (_openBlock(), _createElementBlock("div", null, [
    _createElementVNode("h1", null, "Hello dorld"),
    _createTextVNode(),
...
patchFlag 是模板编译的时候生成的。在编译生成 render 函数的时候会有。

原版vue3案例，我们在[这个网站](https://vue-next-template-explorer.netlify.app/#eyJzcmMiOiI8ZGl2PlxyXG4gIDxoMT5oZWxsbyB3b3JsZDwvaDE+XHJcbiAgPHNwYW4+e3thZ2V9fTwvc3Bhbj5cclxuPGRpdj4iLCJvcHRpb25zIjp7fX0=),可以看template被解析成的render函数的样子。我们先用原版的vue实现下面的功能。渲染出页面。

![image-20220419224250805](https://picture-stores.oss-cn-beijing.aliyuncs.com/img/image-20220419224250805.png)
...
+ block 来收集节点， 为不稳定结构（v-if 和 v-for ）的也创建block节点，实现blockTree 做到靶向更新
+ 优化 （静态提升 属性的提升和虚拟节点的提升，函数的缓存，预解析字符串）
+ 写模板的性能会比直接写h函数更好一些， （可以手写 openBlock createElementBlock  createTextVnode()  patchFlags作用）
+ 每个v-if都会收集openblock，并且节点的key不一样，每次渲染都会重新patch.

### vue3中的ast语法树转化

![image-20220420101155884](https://picture-stores.oss-cn-beijing.aliyuncs.com/img/image-20220420101155884.png)
...
异步组件就是一开始渲染一个空组件，稍后加载完毕之后渲染一个真实组件

实现promise.retry重试方法

```js
let { h, createApp, defineAsyncComponent, render } = Vue
...
let asyncComponent = defineAsyncComponent({
    loader: () => {
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                reject(Component) // 正常情况下，应该在一次加载失败之后，重新加载
            }, 1000)
        })
    },
    timeout: 2000, // 超时时间
    delay: 1000, // 延迟时间, 如果在1000ms内，还没有加载完成，就会显示 LoadingComponent
